...
def sell():
    ...
    if request.method == "POST":
        ...
        # Query database to insert transaction
        db.execute("INSERT INTO transactions (user_id, date, symbol, shareamount, shareprice) VALUES (:user_id, :date, :symbol, :shareamount, :shareprice)",
            user_id = session["user_id"],
            date = date.today(),
            symbol = input_symbol,
            shareamount = int(input_shares),
            shareprice = format(price,".2f"))
